...
// import { getAllBookIds } from "../lib/googleSheet";
...
function Index({ bookIds }) {
  ...
  return (
    <Layout>
      {bookIds.map(({ params: { id } }) => {
        return (
          <div key={id}>
            <Link href={`/books/${id}`}>{id}</Link>
          </div>
        );
      })}
    </Layout>
...
export default function Example() {
  return (
    <Layout>
      <div className="mx-auto mt-1 max-w-7xl bg-white py-6 sm:py-8">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          ...
          >
            {people.map((person) => (
              <li
                key={person.name}
                className="rounded-md p-4 shadow-none transition-shadow hover:shadow-lg"
              >
                <img
